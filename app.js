...
function checkFiles() {
	...
	if (!envExists) {
		createReadStream(join(__dirname, '/lib/template.env')).pipe(createWriteStream(join(__dirname, '/.env')));
	}
}
...
if (!sessionDataDefaultsFileExists) {
	...
	createReadStream(join(__dirname, '/lib/template.session-data-defaults.js')).pipe(createWriteStream(sessionDataDefaultsFile));
}
...
app.use('/nhsuk-frontend', express.static(join(__dirname, 'node_modules/nhsuk-frontend/dist/nhsuk')));
